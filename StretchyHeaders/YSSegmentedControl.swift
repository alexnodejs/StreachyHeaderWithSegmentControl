...
public class YSSegmentedControl: UIView {
    ...
    func onSwipeLeft(recognizer: UISwipeGestureRecognizer) {
        if selectedIndex < items.count - 1 {
            selectItemAtIndex(++selectedIndex)
            delegate?.segmentedControlDidPressedItemAtIndex?(self, index: selectedIndex)
        }
    }
    
    func onSwipeRight(recognizer: UISwipeGestureRecognizer) {
        if selectedIndex > 0 {
            selectItemAtIndex(--selectedIndex)
            delegate?.segmentedControlDidPressedItemAtIndex?(self, index: selectedIndex)
        }
    ...
    private func addSwipeRecognizer() {
        ...
        swipeSelector?.addGestureRecognizer(swipeLeft)
        swipeSelector?.addGestureRecognizer(swipeRight)
    }
    ...
    private func draw () {
        ...
        // selector
        let textWidth = items[0].label.intrinsicContentSize().width
        let textTarget = width / 2 - textWidth / 2
    ...
    private func moveSelectorAtIndex (index: Int) {
        ...
        let textWidth = items[index].label.intrinsicContentSize().width
        let target = width * CGFloat(index)
        let textTarget = target + (width / 2 - textWidth / 2)
